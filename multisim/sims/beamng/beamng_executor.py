...
from sims.beamng.beamng_brewer import BeamNGBrewer

# maps is a global variable in the module, which is initialized to Maps()
from sims.beamng.beamng_car_cameras import BeamNGCarCameras
from sims.beamng.beamng_tig_maps import LevelsFolder, maps
from sims.beamng.beamng_waypoint import BeamNGWaypoint
from sims.beamng.config import BEAMNG_VERSION, MAX_SPEED_BEAMNG, MIN_SPEED_BEAMNG
from sims.beamng.decal_road import DecalRoad
from sims.beamng.simulation_data import SimulationDataRecord
from sims.beamng.simulation_data_collector import SimulationDataCollector
...
from config import BEAMNG_STEP_SIZE
import time
...
class BeamngExecutor:
    ...
    def reset(self, skip_generation: bool = False, road: Road = None) -> None:
        ...
        log.info("[BeamNG Executor] Brewer is initialized.")
        ...
        if not skip_generation:
            ...
            # log.info(F"[BeamngExecutor] Nodes road: {nodes}")
        ...
        log.info("[BeamNG] reset complete")
    ...
    def take_action(self, steering: float, throttle: float = None) -> None:
        ...
        * ``steering``: Rotation of the steering wheel, from -1.0 to 1.0.
        ...
        """
        # print(f"[Executor] steering: {steering} throttle: {throttle}")
        try:
            ...
            self.brewer.beamng.step(BEAMNG_STEP_SIZE, wait=False)
    ...
    def is_game_over(self) -> Tuple[bool, int]:
        ...
        if target_reached:
            print("BeamngExecutor: Target Reached.")
    ...
    def observe(self) -> ObserveData:
        try:
            ...
            # print(f"[BeamNG Executor] camera retrieve takes: {end -start}")
            ...
            # print(f"len array: {len(np.array(img))}")
            # print(f"done: {done}")
            # print(f"info: {info}")
