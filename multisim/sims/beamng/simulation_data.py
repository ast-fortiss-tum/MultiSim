...
class SimulationData:
    ...
    def set(self, params: SimulationParams, road: DecalRoad, states: SimulationDataRecords, info: SimulationInfo = None):
        self.params = params
        self.road = road
        if info:
            self.info = info
        ...
        if self.name is not None:
            ...
            if len(self.images) > 0:
                ...
                for i in range(len(self.images)):
                    ...
                    cv2.imwrite(filename=self.path_root.__str__() + "/images/{}_center.jpg".format(i), img=img_center)
                    cv2.imwrite(filename=self.path_root.__str__() + "/images/{}_left.jpg".format(i), img=img_left)
                    cv2.imwrite(filename=self.path_root.__str__() + "/images/{}_right.jpg".format(i), img=img_right)
