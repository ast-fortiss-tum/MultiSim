...
def get_problem_class(problem_name, *args, **kwargs):
    problem_name = problem_name.lower()

    if problem_name.startswith("bbob-"):
        from pymoo.vendor.vendor_coco import COCOProblem
        return COCOProblem(problem_name.lower(), **kwargs)
    ...
    if problem_name not in PROBLEM:
        raise Exception("Problem not found.")

    PROBLEM_CLASS = PROBLEM[problem_name]

    return PROBLEM_CLASS

def get_labeled_problem(problem_name, labeling_name, *args, **kwargs):
    problem_name = problem_name.lower()
    labeling_name = labeling_name.lower()

    from core.problem_labeled.labelingFunction import label_none, label_box, label_chessboard, label_directions

    labeling = {
        'none': label_none,
        'box': label_box,
        'chessboard': label_chessboard,
        'directions': label_directions
    }

    PROBLEM_CLASS = get_problem_class(problem_name)
    labeling_function = labeling[labeling_name]

    class LABELED_PROBLEM_CLASS(PROBLEM_CLASS):
        def __init__(self):
            super().__init__()

        def _evaluate(self, x, out, *args_eval, **kwargs_eval):
            super(LABELED_PROBLEM_CLASS, self)._evaluate(x, out, *args_eval, **kwargs_eval)
            # label is assigned to each individual
            out["L"] = labeling_function(x, out, *args, **kwargs)

        def pareto_front_filtered(self, *args_pf, **kwargs_pf):
            """
            Provides a filtered Pareto front based on the labeling function.
            :return: a set of points distributed equally on the filtered Pareto front.
            """
            pf = self._calc_pareto_front(*args_pf, **kwargs_pf)
            ...
            x = {}  # x is dummy here
            out["F"] = pf
            labels = labeling_function(x, out, *args, **kwargs)  # labels for the Pareto front points

            pf_filtered = []
            for key, pf_ind in enumerate(pf):
                if labels[key] == 1:
                    pf_filtered.append(pf_ind)

            pf_filtered = np.array(pf_filtered)
            return pf_filtered


    return LABELED_PROBLEM_CLASS()
