...
@dataclass
class ADSMultiSimAgreementProblemDiversePredict(Problem):
    def __init__(self,
                 xl: List[float], 
                 xu: List[float], 
                 scenario_path: str, 
                 ...
                 disagree_classifiers: List= None,
                 threshold_uncertainty: float = TH_PRED,
                 fitness_function_predict: Fitness = None):
        ...
        self.disagree_classifiers = disagree_classifiers
        self.threshold_uncertainty = threshold_uncertainty
        self.fitness_function_predict = fitness_function_predict
        self.time_created = datetime.now().strftime(
            "%d-%m-%Y_%H-%M-%S")
    ...
    def _evaluate(self, x, out, *args, **kwargs):
        ...
        ind_x_predict = []
        ind_x_else = []
        probs_list = [] # predict uncertainties for each individual
        if self.disagree_classifiers is not None:
            is_disagreement_list = []
            for ind,x_i in enumerate(x):
                is_disagreement, probs = disagree_predictor.predict(x_i,
                    predictor_paths=self.disagree_classifiers,
                    problem_name=self.problem_name,
                    threshold=self.threshold_uncertainty)
                is_disagreement_list.append(is_disagreement)  
                probs_list.append(probs)
                if is_disagreement:
                    ind_x_predict.append(ind)
                else:
                    ind_x_else.append(ind)
        else:
            ind_x_else = range(0,len(x))

        try:
            simout_sims = []
            for simulate_function in self.simulate_functions:
                simout_list = []
                # sort first what is predicted as idsagreement or not
                # we run the cheap mock simulator when no "real" simulation required to get some output
                from mock.mock_simulation import MockSimulator
                simout_list_predict = MockSimulator.simulate(x[ind_x_predict],
                                                self.simulation_variables, 
                                                self.scenario_path, 
                                                sim_time=self.simulation_time,
                                                time_step=self.sampling_time)
                
                simout_list_default = simulate_function(x[ind_x_else],
                                                self.simulation_variables, 
                                                self.scenario_path, 
                                                sim_time=self.simulation_time,
                                                time_step=self.sampling_time)
                
                simout_list = simout_list_predict + simout_list_default
                
                # rearrange to preserve order
                rearranged_array = np.empty_like(simout_list)  # Create an array of the same size
                rearranged_array[list(ind_x_predict) + list(ind_x_else)] = simout_list

                simout_list = rearranged_array
        ...
        for i in range(len(x)):
            ...
            for j in range(len(self.simulate_functions)):
                ...
                # right now we store the information twice if we have two sims
                simout.otherParams["predict_uncertainties"] = probs_list[i]
                simout.otherParams["predict_disagreement"] = is_disagreement_list[i]
                if is_disagreement_list[i]:
                    # HACK we use the index to decide if assign a critical or non critical fitness
                    kwargs["order"] = j
                    # when disagreement is predicted fitvalues have also to be predicted
                    fvalues = self.fitness_function_predict.eval(simout, **kwargs)
                else:
