...
def read_disagree_predict(filename):
    import csv
    import re
    ...
    with open(file_path, "r", newline="") as file:
        ...
        for row in reader:
            ...
            if row == []:
                continue
            test_input = row[0]
            th_disagree = row[3].strip().lower() == "true"

            # Convert to float if needed
            # Extract the array from the first column if th_disagree is True
            if th_disagree:
                try:
                    # Ensure correct formatting for string-based arrays
                    if isinstance(test_input, str):
                        
                        test_input = test_input.strip().replace("\n", " ")
                        lst = np.array(re.split(r'\s+', test_input.strip('[]').strip()))
                        lst = lst.astype(float)

                        # Normalize spacing
                        #print(f"Processing: {test_input}")  # Debugging output
                        # test_input = ast.literal_eval(test_input)  # Convert string to list
                    
                    array = np.array(test_input)  # Convert to NumPy array
                    test_input_arrays.append(array)
                except Exception as e:
                    print(f"Error parsing array: {test_input}, Error: {repr(e)}")  # More detailed error output



    # Print results
    print("Extracted arrays where th_disagree is True:")
    for arr in test_input_arrays:
        print(arr)
...
def get_first_last_valid(path_all_tests,
    ...
    if len(evaluation_num_valid) == 0:
        ...
        # subtract the number if disagreement predictions before
        n_subtract = 0
        if len(tests_disagree_predict) > 0:
            for i in range(road_index_first):
                road = pop[i].get("X") 
                #print("road is:", road)
                for road_predict in tests_disagree_predict:
                    #print("road_predict is:", road_predict)
                    if np.array_equal(road, road_predict):
                        print("Road was predicted.")
                        input()
                        n_subtract = n_subtract + 1
                        break
            
        print("n_subtract is:", n_subtract) 
        #input()
    ...
    return  [min(evaluation_num_valid) - n_subtract, max(evaluation_num_valid),  angles_valid[road_index_first], angles_valid[road_index_last]]
...
def get_first_last_valid_all(paths,
    ...
    for j, folder_path in enumerate(paths):
        ...
        disagree_predict_file = f"disagree_predict.csv"
        ...
        for i in selector:
            ...
            ######## predict related
            path_disagree_predict = find_files_with_parent(path + os.sep, 
                                        prefix=disagree_predict_file,
                                        parent_folder="myfolder")[0]
            
            print("path_disagree_predict:", path_disagree_predict)
            
            tests_disagree_predict = read_disagree_predict(path_disagree_predict)
            
            print("disagree predictions:", len(tests_disagree_predict))
            #############
            ...
            # consider disagreement predictions
            max_evals = len(pop) - len(tests_disagree_predict)
