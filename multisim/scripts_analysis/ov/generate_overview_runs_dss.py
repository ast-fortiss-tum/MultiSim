...
def generate_overview_runs(folder_path,
    ...
    if save_folder is None:
        save_folder = os.path.join(folder_path,f"overview_all_qu_dss_{sim}_{combo}/")
    Path(save_folder).mkdir(parents=True, exist_ok=True)

    output_file_name_prefix = f"overview_all_qu_dss_{sim}_{combo}_"

    validation_folder = f"validation_all_qu_dsim_combined_count-3.0_{sim}_{combo}"
