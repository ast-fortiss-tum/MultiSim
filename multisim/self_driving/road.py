...
from config import DISPLACEMENT, NUM_SAMPLED_POINTS, BBOX_SIZE
...
class Road(ABC):
    ...
    def is_valid(self) -> bool:
        no_overlap = RoadPolygon.from_nodes(self.get_concrete_representation(to_plot=True)).is_valid()
        in_box = self.road_bbox.contains(
            RoadPolygon.from_nodes([(cp.x, cp.y, cp.z, self.road_width) for cp in self.control_points[1:-1]])
        )
        #print(f"[Road] no overlap: {no_overlap}")
        #print(f"[Road] in box: {in_box}")
        return no_overlap and in_box
    ...
    @staticmethod
    def mutate_road_simple(index: int, 
                           cp: List[Tuple[float, float, float, float]],
                           displacement = 2) -> List[Tuple[float, float, float, float]]:
        temp = list(cp[index])
    ...
    @staticmethod
    def mutate_road_angle(index: int, 
                           cp: List[Tuple[float, float, float, float]],
                           displacement = 2) -> List[Tuple[float, float, float, float]]:
        temp = list(cp[index])
        print("before mutation: ", temp)
